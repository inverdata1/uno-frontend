...
import { Keyboard, KeyboardAvoidingView, Platform, ScrollView, StyleSheet, TouchableWithoutFeedback, View, useWindowDimensions } from 'react-native';
import BusinessOnboardingStep from '../../../../features/client/businesses/business-onboarding-step';
import ClientPreferencesOnboardingStep, { GOALS_OPTIONS, CATEGORIES_OPTIONS, TAGS_OPTIONS } from '../../../../features/client/onboarding/client-preferences-step';
...
export const RegistrationForm = ({ onComplete }) => {
  ...
  // Render Confirmation Step Summary
    ...
    <View>
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Ionicons
            name={selectedUserType === 'client' ? 'basket' : selectedUserType === 'business' ? 'briefcase' : 'bicycle'}
            size={48}
            color="#ffffff"
          />
        </View>
        <Text style={{
          ...
          {selectedUserType === 'client' ? 'Cliente UNO' : selectedUserType === 'business' ? 'Negocio UNO' : 'Delivery UNO'}
        </Text>
        <Text style={{
      ...
      <View className="space-y-3 mb-6">
        ...
        {/* Selected Client Preferences Summary */}
        {selectedUserType === 'client' && clientPreferences && (
          <View style={{
            backgroundColor: '#ffffff',
            borderWidth: 1,
            borderColor: '#e5e7eb',
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text variant="caption" className="text-gray-500">
                  Personalización inicial para tu cuenta
                </Text>
              </View>
              <Ionicons name="checkmark-circle" size={24} color="#10b981" />
            </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
              {clientPreferences.goals?.map((gId) => {
                const goal = GOALS_OPTIONS.find((g) => g.id === gId);
                if (!goal) return null;
                return (
                  <View key={gId} style={{ backgroundColor: '#fef2f2', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#fecaca', flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, marginRight: 4 }}>{goal.emoji}</Text>
                    <Text style={{ fontSize: 12, color: '#dc2626', fontWeight: '600' }}>{goal.label}</Text>
                  </View>
                );
              })}
              {clientPreferences.categories?.map((cId) => {
                const cat = CATEGORIES_OPTIONS.find((c) => c.id === cId);
                if (!cat) return null;
                return (
                  <View key={cId} style={{ backgroundColor: '#f0f9ff', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#bae6fd', flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, marginRight: 4 }}>{cat.emoji}</Text>
                    <Text style={{ fontSize: 12, color: '#0284c7', fontWeight: '600' }}>{cat.label}</Text>
                  </View>
                );
              })}
              {clientPreferences.subcategories?.map((tId) => {
                const tag = TAGS_OPTIONS.find((t) => t.id === tId);
                if (!tag) return null;
                return (
                  <View key={tId} style={{ backgroundColor: '#fdf4ff', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#f5d0fe', flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, marginRight: 4 }}>{tag.emoji}</Text>
                    <Text style={{ fontSize: 12, color: '#c026d3', fontWeight: '600' }}>{tag.label}</Text>
                  </View>
                );
              })}
            </View>
  ...
  // On web desktop: use responsive card layout without TouchableWithoutFeedback (which steals input focus)
  if (isDesktop) {
    return (
      <WebAuthLayout scrollable={true}>
        <View>
            {/* Logo */}
            ...
            <View className="mb-6">
              ...
              {/* Step 3: Business Info or Client Preferences */}
              ...
              {currentStep === 3 && selectedUserType === 'client' && (
                <ClientPreferencesOnboardingStep
                  preferences={clientPreferences}
                  onPreferencesChange={setClientPreferences}
                />
              )}

              {/* Confirmation Step */}
              {currentStep === totalSteps && renderConfirmationStep()}
            </View>
