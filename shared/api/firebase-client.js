...
/**
 * Main Firebase client that manages resources and routes requests
 * Acts as the central router for all Firebase operations
 */
export class FirebaseClient {
  ...
  async request(method, endpoint, data = null, params = {}) {
    try {
      ...
      console.log(`🔥 Firebase ${method} /${resource}/${action}`, { data, params });

      const result = await resourceHandler.handle(method, action, data, params);

      console.log(`✅ Firebase ${method} /${resource}/${action} completed`);
      return result;

    } catch (error) {
      console.error(`❌ Firebase ${method} ${endpoint} failed:`, error);
      throw error;
    }
  }
